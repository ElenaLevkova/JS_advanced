...
window.addEventListener('DOMContentLoaded', () => {
    const cartItemRender = (good) => {
        return `<div class="cart__product_card"  data-id_product="${good.item.idProduct}">
        <img class="cart__product_card_img" src="${good.item.imgPreview}" alt="foto">
        <div class="cart__product_card_text">
            <div class="cart__product_card_title">
                <a href="product.html"> ${good.item.productName}</a>
            ...
            <p>Price: <span class="cart__product_card_text_red">$${good.item.price}</span></p>
            <p>Color: Red</p>
            <p>Size: Xl</p>
            <div class="cart__product_quantity">
                <p>Quantity:</p>
                <input   data-id_product="${good.item.idProduct}" type="number" value = ${good.quantity} min = "1">
            </div>
    ...
    const pCart = {
        ...
        getCart () {
            console.log('tst', localStorage["goods"], typeof(localStorage["goods"]));
        ...
        setCart (cart) {
            if (typeof localStorage["goods"] === 'undefined') localStorage["goods"] = JSON.stringify([]);
        ...
        addGoods(idGoodNew) {
            ...
            if (goodNew === undefined) 
                this.goodsInCart.push({item:productCatalog.goods.find(item => item.idProduct === idGoodNew), quantity: 1}) //Добавляем новый товар в массив корзины
            else {
    ...
    const cart = {
        ...
        cartBlock: null,
        cartButton: null,
        goodsInCart: pCart.getCart(),
        sumSub: document.querySelector('.sum_sub'),
        sumGtand: document.querySelector('.sum_grand'),
        ...
        render() {
            ...
            if (this.goodsInCart.length) {
                this.goodsInCart.forEach(good => {
                    this.cartListBlock.insertAdjacentHTML('beforeend', cartItemRender(good));
                });
            ...
            console.log('1',cart.goodsInCart);
        },

        clearCart() {
            this.goodsInCart = [];
            pCart.setCart(this.goodsInCart);
            this.init();
        },

        countCartPrice() {
            this.goodsInCart = pCart.getCart();
        ...
        updateCart(event) {
            ...
            pCart.updateGoods(event.target.dataset.id_product, event.target.value);
            this.sumSub.textContent = `$${this.countCartPrice()}`;
            this.sumGtand.textContent = `$${this.countCartPrice()}`;
